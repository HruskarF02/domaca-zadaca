...
class Bug0(Node):
    ...
    def __init__(self):
        ...
        self.bug_algorithm_timer = self.create_timer(0.15, self.bug_algorithm_callback)
        self.goal_x = None
        self.goal_y = None
        self.current_x = None
        self.current_y = None
    ...
    def bug_algorithm_callback(self):
        ...
        #self.calc_y=(self.goal_y/self.goal_x)*self.current_x  
        ...
        if goal_distance < 0.1:
            ...
            self.start_x=self.goal_x-0.02
            self.start_y=self.goal_y-0.02 
        ...
        if self.state == 'MOVE_TO_GOAL':
            ...
            if abs(self.calc_y-self.current_y)<0.1 and (self.b>150 or self.c==1):     
            ...
            if self.fl_sensor_value<0.9:
                self.cmd_vel_msg.linear.x = 0.0     
                self.cmd_vel_msg.angular.z = 0.0
                if abs(self.calc_y-self.current_y)>0.25:          
                    self.state='FOLLOW_OBSTACLE'
    ...
    def move_to_goal(self, goal_angle):
        ...
        if abs(self.angle_diff) > 0.1:
            ...
            if self.angle_diff >0:
                self.cmd_vel_msg.angular.z = 0.30
            else:
                self.cmd_vel_msg.angular.z = -0.30
            
        else: 
                self.cmd_vel_msg.linear.x = 0.30       
                self.cmd_vel_msg.angular.z = 0.0
                self.b=0
        self.cmd_pub.publish(self.cmd_vel_msg)
